// ========== SECAO: PAGINA TIKTOK ==========

import { Link } from 'react-router-dom';
import { ArrowLeft } from 'lucide-react';
import { TikTokProvider, useTikTok } from '../contexts';
import {
  TikTokSearchBar,
  TikTokFilterBar,
  TikTokSelectionControls,
  TikTokVideoGrid,
  TikTokDownloadModal,
} from '../components/tiktok';

/**
 * Conteudo da pagina TikTok
 */
function TikTokContent() {
  const { profile, videos } = useTikTok();

  return (
    <div className="min-h-screen bg-dark-900">
      {/* Header */}
      <header className="bg-dark-800 border-b border-dark-700 sticky top-0 z-40">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            <Link
              to="/"
              className="flex items-center gap-2 px-3 py-2 bg-dark-700 hover:bg-dark-600 text-white rounded-lg transition-colors"
            >
              <ArrowLeft size={18} />
              <span>Menu</span>
            </Link>

            <div className="flex items-center gap-3">
              {/* TikTok Logo */}
              <svg viewBox="0 0 24 24" className="w-8 h-8 text-white" fill="currentColor">
                <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/>
              </svg>
              <h1 className="text-xl font-bold text-white">TikTok Analyzer</h1>
            </div>

            {/* Info do perfil */}
            {profile && (
              <div className="ml-auto text-right">
                <p className="text-white font-medium">@{profile.username}</p>
                <p className="text-dark-400 text-sm">{videos.length} videos</p>
              </div>
            )}
          </div>
        </div>
      </header>

      {/* Main Content */}
      <main className="container mx-auto px-4 py-6">
        {/* Busca */}
        <TikTokSearchBar />

        {/* Filtros e Selecao (so mostrar se tiver videos) */}
        {videos.length > 0 && (
          <>
            <TikTokFilterBar />
            <TikTokSelectionControls />
          </>
        )}

        {/* Grid de Videos */}
        <TikTokVideoGrid />
      </main>

      {/* Modal de Download */}
      <TikTokDownloadModal />
    </div>
  );
}

/**
 * Pagina TikTok com Provider
 */
export function TikTokPage() {
  return (
    <TikTokProvider>
      <TikTokContent />
    </TikTokProvider>
  );
}
